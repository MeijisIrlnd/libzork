...
void dump_robber(archive& ar) {
    ...
    std::transform(robber->hobjs_ob().begin(), robber->hobjs_ob().end(), std::back_inserter(rob_objs), [](ObjectP o) { return o->oid(); });
    ar& rob_objs;
...
void restore_robber(archive& ar) {
    ...
    std::transform(rob_objs.begin(), rob_objs.end(), std::back_inserter(robber_items), [](const std::string& s) { return sfind_obj(s); });
...
/*
 * called from rooms.cpp, bound to the SAVE command.
 * the string it takes seems to just be a filename, doesn't matter if it's not a full path name.
 * SO, TODO: have some kind of way to set the saves dir globally for zork.
 */
