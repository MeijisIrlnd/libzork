...
namespace {
    ...
    ObjectP make_obj(const StringList& syns,
                     const StringList& adj,
                     const char* desc,
                     const std::initializer_list<Bits>& bits,
                     rapplic obj_fun = nullptr,
    ...
    GObjectPtr make_gobj(Bits gbits,
                         const StringList& syns,
                         const StringList& adj,
                         const char* desc,
                         const std::initializer_list<Bits>& bits,
                         rapplic obj_fun = nullptr,
...
#include "gobject.h"
        ...
        std::transform(std::begin(gobjects),
                       std::end(gobjects),
                       std::back_inserter(o),
                       [](const GObjectDefinition& od) {
...
#include "objdefs.h"
        ...
        std::transform(std::begin(objects),
                       std::end(objects),
                       std::back_inserter(o),
                       [](const ObjectDefinition& od) {
...
Object::Object(const std::initializer_list<const char*>& syns,
               const std::initializer_list<const char*>& adj,
               const char* description,
               const std::initializer_list<Bits>& bits,
               rapplic objfun,
               const StringList& cntnts,
               const std::initializer_list<OP>& props)
    : synonyms(syns.begin(), syns.end()),
      adjec(adj.begin(), adj.end()),
      desc(description),
      objfn(objfun) {
    std::transform(cntnts.begin(),
                   cntnts.end(),
    ...
    for (const OP& obj_prop : props) {
        ...
        case ksl_oglobal: _oglobal = (Bits)std::get<int>(obj_prop.value());
            break;
        case ksl_oactor: _oactor = (e_oactor)std::get<int>(obj_prop.value());
            break;
        case ksl_ovtype: _ovtype = (Bits)std::get<int>(obj_prop.value());
            break;
        case ksl_ofmsgs: _melee_func = std::get<OP::melee_func>(obj_prop.value());
            break;
        case ksl_obverb:
...
GObject::GObject(Bits g_bits,
                 const StringList& syns,
                 const StringList& adj,
                 const char* desc,
                 const std::initializer_list<Bits>& _bits,
                 rapplic obj_fun,
                 const std::initializer_list<const char*>& contents,
                 const std::initializer_list<OP>& props)
...
void cleanup_gobjects() {
    // Typedef for std::vector<std::shared_ptr....
    auto& globalObjs = global_objects();
    globalObjs.clear();
}
...
olint_t::olint_t(int v, bool enable, const CEventP& ev, int init_val)
    : _val(v),
      _ev(ev) {
    _ev->ctick(init_val);
}
