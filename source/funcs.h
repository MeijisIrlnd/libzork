...
/*
 * Pushes our string into a second readerwriterqueue, (in this case we're the producer)
 */
...
bool tell_queue(const T& s, uint32_t flags = post_crlf) {
    ...
    auto res{ false };
    do {
        res = e_outputQueue.try_enqueue(stream.str());
    } while (!res);
    return true;
}

/*
 * pushes our string into tty (which is our overriden output stream).
 * tell_pre pushes endl if flags = pre_crlf,
 * tell_post pushes endl if flags = post_crlf
 */
template <typename T>
 ...
 */
int readst_from_queue(std::string& rdbuf, std::string_view prompt);

/*
 * reads a string from cin
 * TODO: Replace this with a pull from a fifo?
 */
