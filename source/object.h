...
public:
    ...
    OP(ObjectSlots os, const PropVal& v)
        : sl(os),
          val(v) {
    }

    explicit OP(ObjectSlots os, int i)
        : sl(os),
          val(i) {
    }

    explicit OP(ObjectSlots os, std::string_view v)
        : sl(os),
          val(std::string(v)) {
    }

    explicit OP(ObjectSlots os, const char* p)
        : sl(os) {
        if (p) val = p;
        else val = (melee_func)nullptr;
    }
